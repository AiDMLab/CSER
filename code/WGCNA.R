...
expFile="TCGAexp.txt"     #输入文件名字
normalCount=44              #正常样品数目
tumorCount=571              #肿瘤样品数目
...
data=avereps(data)#多行取均值
data=log2(data+1)#log
data=data[apply(data,1,sd)>0.5,]#如果基因在所有样品中波动很小,则删掉这些基因,对样本分类不起作用
datExpr0=t(data)#转置
...
###剪切线
abline(h = 20000, col = "red")#20000代表剪切的标准,可以根据情况修改
dev.off()

###删除剪切线以下的样品
clust = cutreeStatic(sampleTree, cutHeight = 20000, minSize = 10)#20000代表剪切的标准,可以根据情况修改
table(clust)
keepSamples = (clust==1)
datExpr0 = datExpr0[keepSamples, ]


###准备临床数据,肿瘤样品标为0.正常样本标为1
...
###邻接矩阵转换
sft #查看最佳power值
softPower =sft$powerEstimate #最佳power值, 14
